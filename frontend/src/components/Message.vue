<template>
    <div class="mail">
        ...
        <div class="message">
            ...
            <div class="to">
                TO: 
                <em class="display"  v-for="person in message.recipients" :key="person">
                    {{ person }}
                </em>
            </div>
            <div class="from">
                FROM:
                <em class="display">{{ message.sender.username }}</em>
                <div v-if="message.cc">
                    CC: 
                    <em class="display"  v-for="person in message.cc" :key="person">
                        {{ person }}
                    </em>
                </div>
            </div>
...
<style>
  ...
  .mail .message .from .display{
    padding: 1% 0.7%;
    font-size: 22px;
    font-style: normal;
    font-weight: bold;
  }
  .mail .message .from div{
    padding: 1.5% 0;
  }
  .mail .message div.content{
    font-size: 18px;
    line-height: 28px;
    ...
    max-height: 500px;
    padding: 1% 0.5%;
    box-shadow: -10px 0 10px -10px rgba(0, 0, 0, 0.5), 0 10px 10px -10px rgba(0, 0, 0, 0.5);
  }
</style>